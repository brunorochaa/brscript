...
#setting frequent stings
	...
	TNI=""$RS"Ferramenta não instalada. Para instalar digite '"$CE""$YS"install"$CE""$RS"'."$CE""
...
function banner
{
	...
	if [[ "$WLANCHECKING" = "" ]]
	then
		echo -e ""$RS" 1"$CE") Ativar "$RS"$WLANN"$CE"$ "$RS"d1"$CE") Desativar "$RS"$WLANN"$CE"$ "$YS"start"$CE") Iniciar modo monitor"
		echo -e ""$RS" 2"$CE") Ativar "$RS"$WLANNM"$CE"$"$RS"d2"$CE") Desativar "$RS"$WLANNM"$CE"$ "$YS"stop"$CE") Parar modo monitor"
	else
		echo -e ""$YS" 1"$CE") Ativar $WLANN$  "$YS"d1"$CE") Desativar $WLANN$      "$YS"start"$CE") Iniciar modo monitor"
		echo -e ""$YS" 2"$CE") Ativar $WLANNM$ "$YS"d2"$CE") Desativar $WLANNM$     "$YS"stop"$CE") Parar modo monitor"
	fi
	echo -e ""$YS" 3"$CE") Mudar endereço MAC  "$YS"d3"$CE") Restaurar MAC Original "$YS"update"$CE") Procurar atualizações"
	if [[ -f /usr/bin/anonym8 ]]
	then
	echo -e ""$YS" 4"$CE") Ativar o anonym8    "$YS"d4"$CE") Desativar o anonym8    "$YS"errors"$CE") Corrigir erros"
	else
	echo -e ""$RS" 4"$CE") Ativar o anonym8    "$RS"d4"$CE") Desativar o anonym8    "$YS"errors"$CE") Corrigir erros"
	fi
	if [[ -f /usr/bin/anonsurf ]]
	then
	echo -e ""$YS" 5"$CE") Ativar o	anonsurf   "$YS"d5"$CE") Desativar o anonsurf  	"$YS"ks"$CE") Atalhos no teclado"
	echo -e ""$YS" 6"$CE") Status do Anonsurf  "$YS"d6"$CE") Reiniciar o anonsurf   "$YS"d"$CE") Me paga um lanche!"
	else
	echo -e ""$RS" 5"$CE") Ativar anonsurf     "$RS"d5"$CE") Desatiovar o anonsurf  "$YS"ks"$CE") Keyboard shortucts"
	echo -e ""$RS" 6"$CE") Status do Anonsurf  "$RS"d6"$CE") Reiniciar o anonsurf   "$YS"d"$CE") Me paga um lanche!"
	fi
	echo -e ""$YS" 7"$CE") Mostrar seu IP público                                   "$YS"s"$CE") Ir para menu de configurações"
	echo -e ""$YS" 8"$CE") Mostrar MAC"
	echo -e ""$YS" 9"$CE") FERRAMENTAS                                              "$YS"15"$CE") Spoof EMAIL"
	if [[ -f /root/ngrok ]]
	then
		echo -e ""$YS"10"$CE") Handshake                                            "$YS"16"$CE") Ngrok na porta reversa"
	else
		echo -e ""$YS"10"$CE") Handshake                                            "$RS"16"$CE") "$RS"Ngrok"$CE" na porta reversa"
	fi
	if [[ -f /usr/local/bin/howdoi ]]
	then
		echo -e ""$YS"11"$CE") Encontrar pin WPS                                    "$YS"17"$CE") Ask (Howdoi tool)"
	else
		echo -e ""$YS"11"$CE") Encontrar pin WPS                                    "$RS"17"$CE") Ask ("$RS"Howdoi"$CE" tool)"
	fi
	echo -e ""$YS"12"$CE") WEP hacking                                              "$YS"18"$CE") MNavegador Auto-exploit"
...
function enable_wlan
{
	...
	ifconfig $WLANN up &>/dev/null && echo -e ""$YS"Pronto"$CE"" && O4=1  || echo -e ""$RS"Error. I can't find your wireless adapter"$CE""
}
function disable_wlan
{
	echo -e "Desativando $WLANN..."
	rfkill unblock wifi &> /dev/null; rfkill unblock all &> /dev/null
	ifconfig $WLANN down &>/dev/null && echo -e ""$YS"Pronto"$CE"" && O4=1 || echo -e ""$RS"Error. I can't find your wireless adapter."$CE""
}
